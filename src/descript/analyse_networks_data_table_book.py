...
#                        continue
    ...
    #G.remove_nodes_from([u for u in G if G.nodes()[u]['Sex'] == 'F']) 
...
#    print(nx.get_node_attributes(G, 'Sex'))
    ...
    men = len([d['Sex'] for (u,d) in G.nodes(True) if d['Sex'] == 'M'])
    women = len([d['Sex'] for (u,d) in G.nodes(True) if d['Sex'] == 'F'])
    Number_Males = men
    ...
    for u,v in G.edges():
        if len(G.nodes()[u]) != 0 and len(G.nodes()[v]) != 0 :
            if G.nodes()[u]['Sex'] == 'M' and G.nodes()[v]['Sex'] == 'M':
                mm += 1
            if G.nodes()[u]['Sex'] == 'F' and G.nodes()[v]['Sex'] == 'F':
                ff += 1
            if G.nodes()[u]['Sex'] == 'M' and G.nodes()[v]['Sex'] == 'F' or G.nodes()[u]['Sex'] == 'F' and G.nodes()[v]['Sex'] == 'M':
                mf += 1
            if G.nodes()[u]['Sex'] == 'M' and G.nodes()[v]['Sex'] == '' or G.nodes()[u]['Sex'] == '' and G.nodes()[v]['Sex'] == 'M':
                mu += 1
            if G.nodes()[u]['Sex'] == 'F' and G.nodes()[v]['Sex'] == '' or G.nodes()[u]['Sex'] == '' and G.nodes()[v]['Sex'] == 'F':
                fu += 1
            if G.nodes()[u]['Sex'] == '' and G.nodes()[v]['Sex'] == '':
                uu += 1       
    ...
    for i,u in enumerate(b_sort):
        if len(G.nodes()[u]['Sex']) > 0 and G.nodes()[u]['Sex'] == 'F':
            BCF += 1
        if i > (len(G))/10:
            break
    for i,u in enumerate(b_sort):
        if len(G.nodes()[u]['Sex']) > 0 and G.nodes()[u]['Sex'] == 'M':
            BCM += 1
        if i > (len(G))/10:
            break
    for i,u in enumerate(b_sort):
        if len(G.nodes()[u]['Sex']) > 0 and G.nodes()[u]['Sex'] == '':
            BCU += 1
        if i > (len(G))/10:
            break
   ...
   ##assign house if unknown
    ...
    #data = 'Narrative', 'N', 'M', '<k>', 'Max_degree', 'Average path length', 'Clustering', 'Transitivity', 'Assortativity', 'Density', 'Giant Component Size (%)']}
    #Stats_standard = {'Titles':['Narrative', 'N', 'M', '<k>', 'Max_degree', 'Average path length', 'Clustering', 'Transitivity', 'Assortativity', 'Density', 'Giant Component Size (%)'],
    #                 'data':[Name, N, M, average_degree, Max_degree, path_length, Random_path_length, CC, Random_Clustering, Transitivity, Assortativity, Density, GC_size]}
    Stats_standard = {'Narrative':Name,'N':N,'M':M,'<k>':average_degree, 'Max_degree':Max_degree, 'Average path length':path_length, 'Random path length':Random_path_length, 'Diameter':diameter, 'Clustering':CC,
                      'Random Clustering': Random_Clustering, 'Transitivity':Transitivity, 'Assortativity':Assortativity, 'Density':Density, 'GC Size (%)':GC_size, 'Average Betweenness':BC_Average, 'Categorical assortativity_Sex': Cat_S, 'Categorical assortativity_House': Cat_H}
    Data_Table_Standard = Data_Table_Standard.append(Stats_standard, ignore_index=True)
    Data_Table_Standard = Data_Table_Standard[['Narrative', 'N', 'M', '<k>', 'Max_degree', 'Average path length', 'Random path length', 'Diameter', 'Clustering', 'Random Clustering', 'Transitivity', 'Assortativity', 'Density', 'GC Size (%)', 'Average Betweenness', 'Categorical assortativity_Sex','Categorical assortativity_House']]
    ...
    if i%250 == 0:
        export_excel_G = Data_Table_Gender.to_excel('Data table gender GoT B_CC' +str(i)+'.xlsx', index=False)
        
      


export_excel_G = Data_Table_Gender.to_excel("out/descript/summary/Data table gender GoT B_CC.xlsx", index=False)
